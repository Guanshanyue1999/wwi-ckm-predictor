...
经10折交叉验证LASSO回归（最优$\lambda$=0.000090），从20个候选特征中保留20个非零系数特征。按绝对值排序，前10位依次为：WHtR\_2023（0.234）、BMI\_2023（-0.143）、WWI\_2023（-0.128）、eGFR\_2023（-0.060）、WC\_2023（0.058）、Sex（0.039）、AGE\_2023（0.022）、DYS\_drugs\_2023（0.021）、LDL\_2023（-0.020）、FBG\_2023（0.019）。整体上，体测指标与代谢/肾功能指标均进入前列，为后续模型训练提供基础。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{lasso_coefficients.png}
\caption{LASSO特征选择系数（非零项）。}
\label{fig:lasso}
\end{figure}

\subsubsection*{4.6.2 模型性能比较}
表\ref{tab:ml_performance}展示了5种机器学习模型对CKM的预测性能比较。XGBoost模型表现最佳，10折交叉验证AUC为$0.917\pm0.012$，测试集AUC为0.926，F1分数0.490。LightGBM与Random Forest性能接近，传统Logistic Regression作为基线模型测试AUC为0.885。DeLong检验显示，XGBoost显著优于Logistic Regression与MLP（均$P<0.001$），与Random Forest/LightGBM差异不显著（$P>0.3$）。
...
\midrule
Logistic Regression & $0.857\pm0.015$ & 0.885 & 0.784 & 0.302 & 0.854 & 0.446 \\
Random Forest & $0.903\pm0.012$ & 0.918 & 0.882 & 0.444 & 0.629 & 0.521 \\
\textbf{XGBoost} & $\mathbf{0.917\pm0.012}$ & \textbf{0.926} & 0.830 & 0.352 & 0.803 & 0.490 \\
LightGBM & $0.915\pm0.011$ & 0.922 & 0.842 & 0.368 & 0.775 & 0.499 \\
MLP Neural Network & $0.878\pm0.017$ & 0.887 & 0.919 & 0.750 & 0.303 & 0.432 \\
\bottomrule
\end{tabular}
...
图\ref{fig:model_evaluation}展示了模型综合评估结果。校准曲线显示，XGBoost模型的预测概率与实际观测概率一致性较好，Brier分数0.101。决策曲线分析表明，在常见阈值概率范围内，XGBoost模型的净收益（Net Benefit）高于"全部治疗"和"全部不治疗"策略，提示该模型在临床决策中具有实用价值。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{comprehensive_evaluation_ckm.png}
\caption{CKM预测模型综合评估。(A) 5种模型的ROC曲线比较，XGBoost（AUC=0.926）表现最佳；(B) 校准曲线显示XGBoost预测概率与实际概率吻合良好；(C) 决策曲线分析表明XGBoost在常见阈值区间净收益较高；(D) 模型性能指标汇总。}
\label{fig:model_evaluation}
\end{figure}

\subsubsection*{4.6.4 SHAP可解释性分析}
图\ref{fig:shap}展示了XGBoost模型的SHAP特征重要性分析结果。Summary Plot显示，WC\_2023的贡献最高（平均$|\text{SHAP}|$=1.69），其次为WHtR\_2023（0.75）、eGFR\_2023（0.51）、Sex（0.46）、FBG\_2023（0.38）。总体来看，中心性肥胖指标与代谢/肾功能指标对模型贡献更大，WWI\_2023仍有贡献但排序靠后。

\begin{figure}[htbp]
\centering
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{shap_summary_ckm.png}
  \caption{SHAP Summary Plot}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{shap_importance_ckm.png}
  \caption{SHAP Feature Importance}
\end{subfigure}
\caption{XGBoost模型的SHAP特征重要性分析。}
\label{fig:shap}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{shap_dependence_wwi_ckm.png}
\caption{WWI的SHAP依赖图。}
\label{fig:shap_wwi}
\end{figure}
...
\textbf{访问地址}：\url{https://wwi-ckm-predictor.streamlit.app/}

\textbf{GitHub源码}：\url{https://github.com/Guanshanyue1999/wwi-ckm-predictor}

\begin{figure}[htbp]
\centering
\fbox{\parbox{0.9\textwidth}{\centering
\vspace{2cm}
\textbf{CKM风险预测网页应用界面}\\[0.5cm]
...
\textbf{在预测建模方面，本研究发现XGBoost模型对CKM的预测性能最佳（AUC=0.926），显著优于传统Logistic回归。这与既往文献中集成学习方法在心血管代谢疾病预测中的优势相一致\cite{Chen2016}。SHAP分析显示，中心性肥胖指标（WC/WHtR）与代谢、肾功能指标对模型贡献更大，WWI仍提供补充信息。决策曲线分析表明，XGBoost模型在常见阈值概率范围内具有临床净收益，支持其作为CKM筛查工具的可行性。}
